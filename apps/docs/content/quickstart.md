...
```javascript
const { UnblockClient } = require("@unblockllm/sdk");
...
| `REDIS_URL` | Optional; Redis for mapping store |
| `DATABASE_URL` | Optional; Postgres for audit logs |

Use **https** for the proxy URL in production. No PII is logged by the SDK or proxy.

## Next steps

- [Security Model](/docs/security-model) — zero-trust and audit
- [SLA](/docs/sla) — overhead targets
